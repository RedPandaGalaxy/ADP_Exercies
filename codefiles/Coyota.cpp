...
class Customer : public Coyota_dealer_affiliated
{
  public:
	  Customer(const Person &p, int c_no, double ac)
		  : person_ptr(&p), customer_number(c_no), account(ac){}
...
class Coyota_dealer
{
 public:
	...
	void register_customer(Customer &c){ customers.push_back(&c);}
...
void write_letter_to_customers(string text)
	{
	   unsigned int i;
		   ...
		   { 
			   cout << endl;
			   cout << customers[i]->write_letter_to(text); 
		   }
    }
...
void print(void)
	   { 
		...
		for(it=employees.begin();it<employees.end();it++)
		 ...
		 cout << endl << " customers: " ;
		for(i=0;i<customers.size();i++)
		  {cout << endl; customers.at(i)->print();}
	   }
